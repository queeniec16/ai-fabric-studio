...
面向纺织与服装设计师的 **Fabric Layer Recolor + Fabric Intelligence + Digital Material Creation** 原型。
...
用户可以分别修改各结构的颜色，同时尽量保留织物纹理。系统会尝试理解
fabric type、structure type 与 yarn behavior，并从确认后的无缝纹理生成
softness-aware Base Color、Normal、Roughness、Height、Metallic 与 Alpha 地图，
在实时 3D 场景中评估数字材质。
...
- 支持 Clone Patch 与 Mirror Patch 两种接缝修补策略
- 可调 Blend Width、Blend Strength、Edge Color Match 与 Texture Preservation
- Before / After 切换可在 2×2、4×4 与 8×8 重复中比较修补效果
- 自动估算 seam quality；风险较高时禁止提前确认
- 只有确认后的 seamless tile 才能进入 PBR map generation
- Fabric Intelligence：自动估算 Knit / Woven / Jacquard / Technical textile
- 尝试输出专业子类，例如 Rib Knit、Plain weave、Twill、Jacquard knit 等
- 显示 Fabric Type、Structure Type、Estimated Yarn Behavior 与 Recommended Softness
- Fabric Softness Controls 使用面料语言控制物理表现
- Soft / Firm Fabric、Yarn Thickness、Flat / Deep Texture、Smooth / Structured、Micro Variation 可实时影响 3D 材质
- 从确认后的 seamless texture 估算 Base Color、Normal、Roughness、Height、Metallic 和 Alpha PBR maps
- Normal 和 Height 生成会根据 softness、yarn thickness、loop depth 与 surface smoothness 调整，避免过硬、过塑料感
- Reflective Yarn / Shine 与 Open Structure Transparency 用于金属纱、亮片、网眼、蕾丝等特殊面料
- 单独预览并下载每张材质地图
- Three.js 实时 3D 材质预览
- Sphere 与 Fabric Plane 两种预览模型
- Repeat 1× / 2× / 4× / 8× / 16× 与 10 / 25 / 50 / 100 cm 材料尺度
- Close-up View 与默认 zoomed-out Full Material View
...
components/MaterialViewer.tsx
  ...
  ├── Repeat and physical material scale controls
  ├── Close-up and full-material camera views
...
components/TileStudio.tsx
  ...
  ├── Clone / Mirror seam repair and before / after comparison
  ├── quality-gated confirmation
...
lib/material-generation.ts
  ...
  ├── Softness-aware Height estimation from local texture contrast
  ├── Softness-aware Normal reconstruction from height gradients
  ├── Roughness and reflective-region Metallic estimation
  └── dark-hole Alpha estimation and PNG rendering

lib/fabric-intelligence.ts
  ├── Heuristic fabric family classification
  ├── Knit / Woven / Jacquard / Technical textile scoring
  ├── Professional sub-category labels
  ├── Estimated yarn behavior
  └── Recommended softness range
...
Fabric intelligence is isolated in
[`lib/fabric-intelligence.ts`](./lib/fabric-intelligence.ts). It is currently an
image-feature heuristic layer and is designed to be replaced by a textile-specific
classifier later.

Material generation is isolated in
[`lib/material-generation.ts`](./lib/material-generation.ts), so AO,
...
- Fabric type recognition is heuristic and should be treated as an early signal, not a certified textile analysis.
- Periodic edge blending prioritizes seam removal and can soften motifs close to tile boundaries.
- Material scale is currently a visual preview control, not calibrated DPI, yarn size,
  or physically measured metadata.
